...
namespace AMS_2 {
    public class DlmsReader {
        ...
        public const int DLMS_READER_BUFFER_SIZE = 512;
        public const int DLMS_READER_MAX_ADDRESS_SIZE = 5;

        private byte[] buffer = new byte[ DLMS_READER_BUFFER_SIZE ];
        private int position;
        private int dataLength;
        private byte frameFormatType;
        private byte[]  destinationAddress = new byte[ DLMS_READER_MAX_ADDRESS_SIZE ];
        private byte destinationAddressLength;
        private byte[] sourceAddress = new byte[ DLMS_READER_MAX_ADDRESS_SIZE ];
        private byte sourceAddressLength;
        ...
        public bool Read( byte data ) {
            ...
            if ( position == 0 && data != 0x7E ) {
                ...
                if ( position == 1 ) {
                    ...
                    destinationAddressLength = (byte)GetAddress( 3, destinationAddress, 0, DLMS_READER_MAX_ADDRESS_SIZE );
                    if ( destinationAddressLength > 3 )
                        Clear();
                    return false;
                } else if ( sourceAddressLength == 0 ) {
                    // Capture the source address
                    sourceAddressLength = (byte)GetAddress( 3 + destinationAddressLength, sourceAddress, 0, DLMS_READER_MAX_ADDRESS_SIZE );
        ...
        public int GetRawData( byte[] dataBuffer, int start, int length ) {
        ...
        private int GetAddress( int addressPosition, byte[] addressBuffer, int start, int length ) {
